...
public class EntityMapper {
    ...
    public static NewsDTO toNewsDTO(News news) {
        ...
        if (news.getComments() != null && org.hibernate.Hibernate.isInitialized(news.getComments())) {
            dto.setComments(news.getComments().stream()
                    .map(EntityMapper::toCommentDTO)
                    .collect(Collectors.toList()));
        }

        if (news.getTags() != null && org.hibernate.Hibernate.isInitialized(news.getTags())) {
